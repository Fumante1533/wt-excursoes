                ...
                <div className="space-y-8">
                    {posts.map(post => (
                        <div key={post.id} className="bg-white dark:bg-zinc-900 rounded-2xl shadow-lg overflow-hidden cursor-pointer hover:shadow-xl transition-shadow" onClick={() => onNavigate('blogPost', { postId: post.id })}>
                            <img src={withUploadCacheBust(post.imageUrl) || 'https://via.placeholder.com/800x400'} alt={post.title} loading="lazy" decoding="async" className="w-full h-64 object-cover" />
                            <div className="p-6">
